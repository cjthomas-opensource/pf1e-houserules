...
\textbf{Statistics have been adjusted} from the SRD versions.
...
%
\subsection{Lamps, Lanterns, and Torches}

The Light and Daylight spells last 1 hour per level if cast on objects made
from, or plated with, gold. Objects with gold-plated fiddly-bits intended
for use with the Light cantrip are everywhere:

\begin{itemize}
%
\item An object with lantern, head-lamp, or flashlight form factor costs
20~sp. A stand-lamp costs 30~sp.
%
\item These shed light in all directions (lantern, stand-lamp) or in a 180
degree arc (head-lamp, flashlight). For an extra 10~sp, you can get a version
with a reflector that sheds light in a 60~degree cone for twice the distance.
%
\item The ``light'' cantrip has a base range of 20 feet; the ``daylight''
spell has a base range of 60 feet. Both will shed dim light to twice that
distance.
\end{itemize}

Mundane light sources have been almost entirely superseded by magical ones.
For comparison, available mundane sources are:

\begin{itemize}
\item Candles burning tallow (cheap) or wax (expensive). No wind protection,
so indoor use only.
\item Clay lamp burning vegetable oil. No wind protection, so indoor use only.
\item Portable lantern. This is a candle or clay lamp in a wind-proof
ventilated container (usually made of perforated sheet metal).
\item Metal and glass desk lamp burning vegetable oil. These are not usually
portable. (The real-life equivalent is an Argand lamp.)
\item Portable metal and glass lanterns using various distillates and
alchemically modified oils are known, but only as curiosities (magic is
cheaper and more reliable). (The equivalent of early kerosene lamps.)
\end{itemize}

Candles are available but mostly only see niche uses (self-melting sealing
wax sticks, or for ceremonial/festival uses). Clay lamps, and portable
lanterns based on candles or clay lamps, are uncommon and are generally art
pieces (often again for ceremonial/festival use). More sophisticated
combustion lamps are made-to-order (as art pieces, toys for the rich, or
both).
%
