...
%
\subsection{Enlarge Person}

This spell works on any creature, not just humanoids. Size is increased by
one step, to a maximum of ``Large''. Attack rolls, AC, CMB, CMD, and other
statistics are adjusted for size per the SRD.

There is a version that's SL3 that increases size by two steps, to a maximum
of ``Huge''.
%
...
Mage Hand's weight limit scales with the caster's size; i.e. 5~lbs for a
size Medium caster, about half a pound for a size Small caster, about 40~lbs
for a size Large caster, and so forth.

If the caster is under a size-changing enchantment (such as Enlarge Person or
Alter Self), the larger of the two sizes (original and changed) applies. A
human reduced to the size of a kobold and a kobold enlarged to the size of a
human can both lift about 5~lbs with Mage Hand.
%
...
with a radius chosen by the caster (up to 30'), or the interior of a
building (or well-defined portion thereof) with up to 10,000 square feet of
floor area (400 5' squares).
...
Permanently enchanting a building (or part of a building) with this effect
requires making an enchanted device and running waveguides along the surface
of the area to be protected. This is typically done for the homes of wealthy
aristrocrats and for vaults.
...
light draft animals. There is a variant that conjures riding dogs or ponies
for size S creatures.
...
%
\subsection{Reduce Person}

This spell works on any creature, not just humanoids. Size is decreased by
one step. Attack rolls, AC, CMB, CMD, and other statistics are adjusted for
size per the SRD. The original size may be no larger than ``Large''.

There is a version that's SL3 that decreases size by two steps, with an
original size no larger than ``Huge''.

Since this spell allows a Fortitude save, and Large and Huge creatures tend
to have very good Fortitude saves, using this spell on an unwilling Large or
Huge target is difficult.
%
...
person making the call can't tell if the call was rejected or if the
...
%
\subsection{Calm Wind}

Druid 2, Magus 2, Wizard, 2

V/S, close (25' + 5/2 levels), 1 minute per level, 30' radius sphere

This calms the air within the affected area; winds of up to gale force become
still. When targetting wind produced by magic (such as Wind Wall), the
caster must succeed in a Dispel Check (per Dispel Magic) to suppress the
magically created wind within the area of effect.
%
...
V/S, touch, 1 day per caster level; fortitude save negates

The subject of the spell can neither conceive a child nor cause their
partner to conceive a child for the duration of the spell. This has no
effect on pregnancies already in progress.

This is widely available for public use, and is sometimes used in animal
husbandry as well (though usually the opposite effect is desired).
%
...
Cleric 1, Magus 1, Wizard 1

touch, 1 weapon or 10 projectiles, 1 hour per caster level

This makes a normally-lethal weapon deal nonlethal damage.
...
\item Anyone passing through the boundary produces light and sound to alert
the occupants of the intrusion. This is usually a flash and crackle.
\end{itemize}
...
from the Immaterial Realm clothed in magically conjured bodies'' cannot
