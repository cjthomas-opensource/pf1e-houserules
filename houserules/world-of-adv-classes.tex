...
\begin{itemize}
\item Bards can't use most magic item creation feats. Case-by-case exceptions
can be made for Wondrous Items that are activated by performance, adjudicated
by the DM.
\item Bards can \textit{use} spell-completion items (such as scrolls) and
spell-trigger items (such as wands), but these require a Use Magic Device
check (with a slightly easier DC, per the modified skill description).
\end{itemize}
%
...
Since a very large number of spells have been invented (any spell that
someone thought would be useful), you can make up a spell effect and ask
the DM if it would be available as a spell gained at character creation or
during level-up. Any such spells are subject to DM adjudication.
...
`Speak With Master'', and ``Speak With Animals'' effects are also gained (but
...
Rogue sneak attack damage scales with size. Die size is d6 for size Medium,
d4 for size Small, and so forth.
%
...
\begin{itemize}
\item Sorcerers can't use the ``scribe scroll'' feat. They can use most
other magic item creation feats.
\item Sorcerers can \textit{use} spell-completion items (such as scrolls),
but these require a Use Magic Device check (with a slightly easier DC, per
the modified skill description). Spell-trigger items (such as wands) can be
used without a check as normal.
\end{itemize}
%
